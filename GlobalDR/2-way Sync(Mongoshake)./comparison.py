...
def check(src, dst):
    ...
    #The database_names is deprecated from 3.7 onwards and been replaced by list_database_names()
    dstDbNames = dst.conn.list_database_names()
    if configure[INCLUDE_DBS] or configure[INCLUDE_COLLS]:      
        include_flag = 1
    ...
    for db in srcDbNames:
        ...
        if include_flag ==0 :
            ...
            srcColls = [coll for coll in srcColls if coll in configure[INCLUDE_COLLS] and srcColls.count(coll) > 0]
            dstColls = [coll for coll in dstColls if coll in configure[EXCLUDE_COLLS] and dstColls.count(coll) > 0]
...
def usage():
    print('|------------------------------------------------------------------------------------------------------------------------------------------------------------------------|')
    print("| Usage_with --exclude: ./comparison.py --src=localhost:27017/db? --dest=localhost:27018/db? --count=10000 (the sample number) --excludeDbs=admin,local --excludeCollections=system.profile --comparisonMode=sample/all/no (sample: comparison sample number, default; all: comparison all data; no: only comparison outline without data)  |")
    print('|------------------------------------------------------------------------------------------------------------------------------------------------------------------------|')
    print("| Usage_with --include: ./comparison.py --src=localhost:27017/db? --dest=localhost:27018/db? --count=10000 (the sample number) --includeDbs=test,itmes ( sample dbs )  --includeCollections=test.users --comparisonMode=sample/all/no (sample: comparison sample number, default; all: comparison all data; no: only comparison outline without data)  |")
    print('|------------------------------------------------------------------------------------------------------------------------------------------------------------------------|')
...
if __name__ == "__main__":
    opts, args = getopt.getopt(sys.argv[1:], "hs:d:n:e:x:i:n", ["help", "src=", "dest=", "count=", "excludeDbs=", "excludeCollections=", "comparisonMode=","includeDbs=","includeCollections="])
    ...
    for key, value in opts:
        ...
        if key in ("-e", "--excludeDbs"):
            if key in ("-i","--includeDbs"):                                # To check if include option exist
